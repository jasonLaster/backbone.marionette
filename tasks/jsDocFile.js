...
_.extend(JsDocFilesTask.prototype, {
  ...
  compileJsDoc: function(file, filepath) {
    ...
    _.each(json.functions, function(value, name) {
      json.functions[name] = this.parseBody(value);
    }, this);

    _.each(json.properties, function(value, name) {
      json.properties[name] = this.parseBody(value);
    }, this);
        
    json.constructor = this.parseBody(json.constructor);
  ...
  **/  
  parseBody: function(value) {
    ...
    result.description = this.parseDox(result.description);
    return result;
  },
